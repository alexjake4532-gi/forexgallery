...
const entryFormEyebrow = document.querySelector("#entryFormEyebrow");
const entryFormTitle = document.querySelector("#entryFormTitle");
...
function resetEntryForm() {
  editingEntryId = null;
  removedImagePaths = new Set();
  entryForm.reset();
  existingImages.innerHTML = "";
  existingImages.classList.add("hidden");
  entryFormEyebrow.textContent = "New Setup";
  entryFormTitle.textContent = "New entry";
  document.querySelector("#saveEntryButton").textContent = "Save entry";
...
function renderExistingImages(entry) {
  const rows = entry.images.map((image, index) => `
    <div class="existing-image-row" data-existing-image-path="${escapeHtml(image.path)}">
      <img src="${image.url}" alt="${escapeHtml(entry.pair)} existing screenshot ${index + 1}" />
      <span>Screenshot ${index + 1}</span>
      <button type="button" class="danger-button" data-remove-existing-image="${escapeHtml(image.path)}">Remove</button>
    </div>
  `);

  existingImages.innerHTML = rows.join("");
  existingImages.classList.toggle("hidden", rows.length === 0);
}
...
function openEditEntryForm(entryId) {
  ...
  entryFormEyebrow.textContent = "Edit Setup";
  entryFormTitle.textContent = "Edit entry";
  document.querySelector("#saveEntryButton").textContent = "Update entry";
  document.querySelector("#pairInput").value = entry.pair || "";
  document.querySelector("#directionInput").value = entry.direction || "buy";
  document.querySelector("#entryPriceInput").value = entry.entry_price ?? "";
  document.querySelector("#stopLossInput").value = entry.stop_loss ?? "";
  document.querySelector("#takeProfitInput").value = entry.take_profit ?? "";
  document.querySelector("#notesInput").value = entry.notes || "";
  renderExistingImages(entry);
      ...
      <article class="entry-card">
        ...
        <div class="entry-content">
          ...
          <div class="entry-actions">
            ...
            <button type="button" class="ghost-button" data-edit-entry-id="${escapeHtml(entry.id)}">Edit</button>
...
openEntryFormButton.addEventListener("click", openCreateEntryForm);
...
entriesGrid.addEventListener("click", (event) => {
  ...
  if (editButton) {
    openEditEntryForm(editButton.dataset.editEntryId);
    return;
  }
...
editDetailButton.addEventListener("click", () => {
  if (activeDetailEntryId) {
    openEditEntryForm(activeDetailEntryId);
  }
});
...
existingImages.addEventListener("click", (event) => {
  ...
  const path = removeButton.dataset.removeExistingImage;
  const row = removeButton.closest("[data-existing-image-path]");

  if (removedImagePaths.has(path)) {
    removedImagePaths.delete(path);
...
entryForm.addEventListener("submit", async (event) => {
  ...
  const entryBeingEdited = editingEntryId ? entryCache.get(editingEntryId) : null;
  const keptExistingPaths = entryBeingEdited
    ? getEntryImagePaths(entryBeingEdited).filter((path) => !removedImagePaths.has(path))
    : [];
  ...
  setMessage(entryMessage, editingEntryId ? "Updating entry..." : "Saving entry...");

  const finalImagePaths = [...keptExistingPaths, ...uploadedPaths];
  ...
  const saveRequest = editingEntryId
    ? supabaseClient.from("trade_entries").update(entryPayload).eq("id", editingEntryId)
    : supabaseClient.from("trade_entries").insert(entryPayload);

  const { error: insertError } = await saveRequest;
  ...
  if (removedImagePaths.size) {
    await supabaseClient.storage
      .from(tradeBucket)
      .remove(Array.from(removedImagePaths));
  }

  setMessage(entryMessage, editingEntryId ? "Entry updated." : "Entry saved.");
  resetEntryForm();
  toggleEntryForm(false);
  loadEntries();
});
