...
async function deleteEntry(entryId, imagePath, pair) {
  const confirmed = window.confirm(`Delete ${pair || "this entry"}? This removes the entry and its screenshot.`);
        ...
        <article class="entry-card">
          ...
          <div class="entry-content">
            ...
            <div class="entry-actions">
              ${imageUrl ? `<button type="button" class="ghost-button" data-viewer-src="${imageUrl}" data-viewer-title="${escapeHtml(entry.pair)} ${escapeHtml(entry.direction)}">View</button>` : ""}
              <button type="button" class="danger-button" data-delete-entry-id="${escapeHtml(entry.id)}" data-delete-image-path="${escapeHtml(entry.image_path)}" data-delete-pair="${escapeHtml(entry.pair)}">Delete</button>
            </div>
          </div>
        </article>
      `;
...
entriesGrid.addEventListener("click", (event) => {
  ...
  if (deleteButton) {
    deleteEntry(
      deleteButton.dataset.deleteEntryId,
      deleteButton.dataset.deleteImagePath,
      deleteButton.dataset.deletePair
    );
    return;
  }
